...
class ConviteAcessoSerializer(serializers.ModelSerializer):
    prestador_nome = serializers.CharField(
        source='prestador.nome_artistico', read_only=True
    )
    ...
    class Meta:
        model = ConviteAcesso
        fields = [
            'id', 'prestador', 'prestador_nome', 'email', 'tipo',
            'permissoes_portal', 'status', 'criado_por', 'criado_por_nome',
            'criado_em', 'expira_em', 'usado_em',
        ]
        read_only_fields = [
            'id', 'status', 'criado_em', 'expira_em', 'usado_em',
            'prestador_nome', 'criado_por_nome',
        ]
...
class EmitirConviteSerializer(serializers.Serializer):
    ...
    permissoes_portal = serializers.JSONField(
        required=False, default=PERMISSOES_ASSESSORIA_DEFAULT
    )
...
class VinculoPrestadorSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'usuario', 'usuario_email', 'usuario_nome',
            'prestador', 'prestador_nome', 'tipo',
            'permissoes_portal', 'ativo', 'criado_em',
        ]
