...
class TokenService:
    ...
    def get_token_ativo(self) -> str:
        token = ConfiguracaoMeta.get("meta_access_token") or settings.META_ACCESS_TOKEN
        if not token:
        ...
        expirando = expira_em and expira_em > 0 and (expira_em - agora) < SETE_DIAS

        if expirado or expirando:
            logger.warning(f"Token {'expirado' if expirado else 'expirando'} — renovando...")
            try:
...
class HealthScoreService:
    ...
    def calcular(self, kpis: dict) -> dict:
        ...
        if not tem_video:
            ...
            score_bruto = scores["cpm"] * 0.35 + scores["frequencia"] * 0.10 + scores["ctr"] * 0.10
    ...
    def salvar(self, prestador: Prestador, resultado: dict, kpis: dict) -> HealthScore:
        ...
        prestador.save(update_fields=["health_score", "health_status", "atualizado_em"])
        logger.info(f"HS salvo: {prestador} → {resultado['score']} ({resultado['status']})")
        return hs
...
class MetaSyncService:
    ...
    def sincronizar(self, prestador: Prestador, inicio: str, fim: str) -> dict:
        ...
        if kpis.get("thruplay", 0) > 0 and not kpis.get("video_p25"):
            ...
            if retencao.get("graph"):
                kpis.update(
                    self._calcular_retencao_do_graph(retencao["graph"], kpis.get("video_3s", 0))
                )
    ...
    def _extrair_kpis(self, raw: dict) -> dict:
        ...
        cost_per_result = self._action_val(raw.get("cost_per_action_type", []), MESSAGE_TYPES)
    ...
    def _agregar_video_ads(self, ads: list) -> dict:
        ...
        totais = {"video_p25": 0, "video_p50": 0, "video_p75": 0, "video_p100": 0}
        for ad in ads:
            totais["video_p25"] += self._video_val(ad.get("video_p25_watched_actions"))
            totais["video_p50"] += self._video_val(ad.get("video_p50_watched_actions"))
            totais["video_p75"] += self._video_val(ad.get("video_p75_watched_actions"))
            totais["video_p100"] += self._video_val(ad.get("video_p95_watched_actions"))
        return totais
    ...
    def _calcular_retencao_do_graph(self, graph: dict, video_3s: int) -> dict:
        ...
        keys = sorted(int(k) for k in graph)
        max_key = keys[-1]
    ...
    def _action_val(self, actions: list, tipos: list) -> float:
        for tipo in tipos:
            hit = next((a for a in actions if a.get("action_type") == tipo), None)
            if hit:
