...
class ChatSessao(models.Model):
    ...
    prestador = models.ForeignKey(
        "prestadores.Prestador", on_delete=models.CASCADE, related_name="sessoes_chat"
    )
...
class ChatMensagem(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    sessao = models.ForeignKey(ChatSessao, on_delete=models.CASCADE, related_name="mensagens")
...
class RoteiroFinal(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    prestador = models.ForeignKey(
        "prestadores.Prestador", on_delete=models.CASCADE, related_name="roteiros"
    )
    sessao = models.ForeignKey(
        ChatSessao, on_delete=models.SET_NULL, null=True, blank=True, related_name="roteiros"
    )
