...
class PrestadorListSerializer(serializers.ModelSerializer):
    ...
    responsavel_nome = serializers.CharField(
        source='responsavel.get_full_name',
        read_only=True
    )
    ...
    class Meta:
        model = Prestador
        fields = [
            'id', 'nome_artistico', 'categoria', 'fase',
            'cidade', 'estado', 'health_score', 'health_status',
            'responsavel_nome', 'atualizado_em'
        ]
...
class PrestadorSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'criado_em',
                            'atualizado_em', 'health_score', 'health_status']
...
class PortalPrestadorSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'nome_artistico', 'categoria', 'fase',
            'cidade', 'estado', 'instagram',
            'health_score', 'health_status',
            'health_score_atual', 'total_sessoes',
            'roteiros_aprovados', 'atualizado_em',
        ]
    ...
    def get_total_sessoes(self, obj):
        return obj.sessoes_chat.filter(
            status__in=['ativa', 'finalizada']
        ).count()
