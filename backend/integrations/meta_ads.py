...
INSIGHT_FIELDS = ",".join([
    ...
    "cost_per_inline_link_click", "clicks", "ctr",
...
class MetaAdsClient:
    ...
    def renovar_token(self) -> str:
        ...
        if not res.ok or 'access_token' not in data:
            raise MetaAPIError(
                f"Falha ao renovar token: {data.get('error', {}).get('message')}")
    ...
    def get_insights_conta(self, account_id: str, time_range: dict) -> dict:
        ...
        if not data.get('data'):
            logger.warning(
                "time_range sem dados — usando date_preset=last_30d")
        ...
        try:
            ...
            for video_id in video_ids:
                try:
                    ...
                    entry = next(
                        (d for d in res.get('data', [])
                         if d.get('name') == 'total_video_retention_graph'),
                        None
                    )
                    graph = entry.get('values', [{}])[0].get(
                        'value', {}) if entry else {}
                    if graph:
